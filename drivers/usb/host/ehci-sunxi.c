...
static struct sunxi_ehci_hcd {
	...
	[0] = {
		...
		.usb_clk_mask = BIT(USB_CLK_OFFSET_PHY1_RST) | BIT(USB_CLK_OFFSET_PHYGATE),
		.ahb_clk_mask = BIT(AHB_GATE_OFFSET_USB_EHCI0),
		.gpio_vbus = CONFIG_USB_VBUS0,
	...
	[1] = {
		...
		.usb_clk_mask = BIT(USB_CLK_OFFSET_PHY2_RST) | BIT(USB_CLK_OFFSET_PHYGATE),
		.ahb_clk_mask = BIT(AHB_GATE_OFFSET_USB_EHCI1),
		.gpio_vbus = CONFIG_USB_VBUS1,
		.csr = (void*) SUNXI_USB_CSR,
		.irq = 40,
		.id = 2,
	}
...
static void sunxi_ehci_enable(struct sunxi_ehci_hcd *sunxi_ehci)
{
	...
	setbits_le32(&ccm->pll6_cfg, CMM_PLL6_CTRL_EN);

	setbits_le32(&ccm->usb_clk_cfg, sunxi_ehci->usb_clk_mask);
...
static void sunxi_ehci_disable(struct sunxi_ehci_hcd *sunxi_ehci)
{
	...
	clrbits_le32(&ccm->usb_clk_cfg, sunxi_ehci->usb_clk_mask);

	clrbits_le32(&ccm->pll6_cfg, CMM_PLL6_CTRL_EN);
}
